...
const PetList = ({setSelectedPet}) => {
  ...
  const petList = data
    ...
    .map((pet) => <PetItem setSelectedPet={setSelectedPet} pet={pet} key={pet.id} />);
  return (
    <>
      <div className="bg-[#F9E3BE] flex flex-col justify-center items-center ">
        <div className="w-[76vw] flex h-[30px] mb-[30px] mt-[30px]">
          ...
          <button className="ml-auto w-[25%] px-3 py-2 rounded-md text-sm md:text-xl border border-black  flex justify-center items-center bg-green-400 hover:bg-green-600"
          onClick={() => {refetch()}}
          >
            Get all pets
            </button>
